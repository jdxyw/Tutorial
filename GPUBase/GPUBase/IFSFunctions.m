...
-(DataPoint) iFun1:(DataPoint)point{
    ...
    temp=[self Sinusodial:point];
    p.p.x+=temp.p.x*0.2;
    p.p.y+=temp.p.y*0.2;
    
    temp=[self Spherical:point];
    p.p.x+=temp.p.x*0.2;
    p.p.y+=temp.p.y*0.2;
    
    temp=[self Swirl:point];
    p.p.x+=temp.p.x*0.3;
    p.p.y+=temp.p.y*0.3;
    
    temp=[self FishEye:point];
    p.p.x+=temp.p.x*0.3;
    p.p.y+=temp.p.y*0.3;
...
-(DataPoint) iFun2:(DataPoint)point{
    ...
    temp=[self Handkerchief:point];
    p.p.x+=temp.p.x*0.3;
    p.p.y+=temp.p.y*0.3;
    
    temp=[self Polar:point];
    p.p.x+=temp.p.x*0.3;
    p.p.y+=temp.p.y*0.3;
...
-(DataPoint) iFun3:(DataPoint)point{
    ...
    temp=[self Heart:point];
    p.p.x+=temp.p.x*0.1;
    p.p.y+=temp.p.y*0.1;
    
    temp=[self HorseShoe:point];
    p.p.x+=temp.p.x*0.3;
    p.p.y+=temp.p.y*0.3;
...
-(DataPoint) iFun4:(DataPoint)point{
...
-(DataPoint) iFun5:(DataPoint)point{
...
-(DataPoint) iFun6:(DataPoint)point{
    ...
    p.red=([[[color objectAtIndex:4] objectAtIndex:0] floatValue] *255+point.red)/2.0;
    p.green=([[[color objectAtIndex:4] objectAtIndex:1] floatValue]*255+point.green)/2.0;
    p.blue=([[[color objectAtIndex:4] objectAtIndex:2] floatValue]*255+point.blue)/2.0;
...
-(DataPoint) caculate:(DataPoint)point
{
    ...
    switch (index) {
        case 0:
            return [self iFun1:point];
            break;
        case 1:
            return [self iFun2:point];
            break;
        case 2:
            return [self iFun3:point];
            break;
        case 3:
            return [self iFun4:point];
            break;
        case 4:
            return [self iFun5:point];
            break;
        case 5:
            return [self iFun6:point];
            break;
