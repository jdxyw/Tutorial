...
void main(void)
{
    ...
    vec2 z  = p*vec2(2.66,2.0)+vec2(0.0,0.0);
    vec2 p1=vec2(-0.6,0.4);
    for( int i=0; i<64; i++ )
    {
        z = power_of_comp(z+cc,3.0)-power_of_comp(z+cc,2.0);
    ...
    float color = log(log(sqrt(sqrt(dmin))))*0.7;
    ...
    gl_FragColor = vec4(hsl2rgb(color1*0.2+color2*0.2+color3*0.3+color4*0.35,1.0,0.7),1.0);
}