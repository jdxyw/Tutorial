...
void main(void)
{
    vec2 p = -abs(-1.0 + 2.0 * gl_FragCoord.xy / resolution.xy);
    ...
    for( int i=0; i<64; i++ )
    {
        z = power_of_comp(power_of_comp(z,3.0)+vec2(1.0,0.0),0.5)+cc;
        float m2 = dot(z,z);
        if( m2>100.0 ) break;
        dmin=min(dmin,m2);
        ...
        dmin=min(dmin,m4);
        float m7=abs((z.x-1.1)*(z.y+1.1));
        dmin=min(dmin,m7);
     }

    float color = log(sqrt(dmin))*0.7;
    gl_FragColor = vec4(hsl2rgb(color*0.5+(1.8-color)*0.12,1.0,0.7),1.0);
}