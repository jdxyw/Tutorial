...
void main(void)
{
    ...
    vec2 z  = p*vec2(1.33,1.0)+vec2(-1.6,0.0);
    vec2 p1=vec2(-0.6,0.4);
    for( int i=0; i<64; i++ )
    {
        z = divide_comp(power_of_comp(z+vec2(1.0,0),2.0)+cc,z);
    ...
    gl_FragColor = vec4(hsl2rgb(color1*0.1+color*0.2+color3*0.3+color4*0.4,1.0,0.7),1.0);
