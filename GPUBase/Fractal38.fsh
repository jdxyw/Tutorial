...
void main(void)
{
    ...
    for( int i=0; i<32; i++ )
    {
        vec2 temp=power_of_comp(z,2.0);
        
        z = divide_comp(2.0*multip_comp(temp,z)+vec2(1.0,0.0)-cc,3.0*temp-cc);
    ...
    gl_FragColor = vec4(hsl2rgb(color*0.2+d1*0.2+d2*0.1+d3*0.1-d5*0.2,1.0,0.7),1.0);
}